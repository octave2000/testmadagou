...
const Properties = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteProperty({ id: propertyToDelete.id }).unwrap();
      handleDeleteClose();
    } catch {
  ...
  const sortedProperties = managerProperties
    ? [...managerProperties].sort((a, b) => {
  ...
  if (error) return <div>{t("dashboard.errorLoadingProperties")}</div>;
